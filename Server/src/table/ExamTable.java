...
public class ExamTable {
    ...
    public static final String DELETE_EXAM_BY_ID = "DELETE FROM " + TABLE_NAME + "WHERE "
            + EXAM_ID_COLUMN + " = ?;";
    public static final String GET_UPCOMING_EXAMS = "SELECT * FROM " + TABLE_NAME
            + " WHERE " + PROCTOR_ID_COLUMN + " = ? AND NOT (" + START_TIME_COLUMN + " > ? OR "
            + END_TIME_COLUMN + " < ?);";
    public static final String GET_UPCOMING_EXAMS_STUDENT = "SELECT * FROM " + TABLE_NAME
            + " WHERE " + COURSE_ID_COLUMN +" IN (" + "SELECT " + COURSE_ID_COLUMN + " FROM " + ENROLLMENT_TABLE
            + " WHERE " + USER_ID_COLUMN + " = ? ) AND WHERE " + START_TIME_COLUMN + " > ?";
